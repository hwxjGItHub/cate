<template>
  <div class="details">
    <div class="header">
      ...
      <img :src="melas.icon" class="icon">
      <span>{{melas.name}}</span>
      <span class="attention">关注</span>
    </div>
    <div class="banner" ref="banner">
      <div>
        <div class="img">
          <img :src="melas.albums">
          <div class="name">{{melas.title}}</div>
        </div>
        <div class="abstract">
          <span>简介：</span>
          {{melas.imtro}}
        ...
        <div class="content">
          <div class="content_set">
            <mu-list>
              ...
              <mu-divider></mu-divider>
              <mu-list-item button>
                <mu-list-item-title style="font-size: 15px;">{{melas.ingredients}}</mu-list-item-title>
              </mu-list-item>
              <mu-divider></mu-divider>
              <div class="sheet">{{melas.burden}}</div>
              <mu-divider></mu-divider>
            </mu-list>
          </div>
        </div>
        <div class="practice">
          ...
          <div class="step" v-for="(data,index) in melas.steps" :key="index">
            <p>{{data.step}}</p>
